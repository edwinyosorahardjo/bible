...
$(function(){
    ...
    // map library to JsonBible
    var $bible = JsonBible;

    // init Bible
    $bible.Init('../db/').then(d=>{
      //console.log(d);
      initView(d, true);
      //create_spritz();
    }).then(initSticky);
    ...
    function onBibleChange(bible){
      ...
      var dfd = $.Deferred();
      // load Bible Info
      $.when($bible.GetBooks()).then(books => {
    ...
    function onBookChange(book){
      ...
      model.chapters = $bible.GetChapters(model.bookIdx);
    ...
    function onChapterChange(chapterIdx){
      ...
      var _b = model.bible;
      // load passages
      $.when($bible.GetPassages(model.bookIdx+1, model.chapterIdx+1)).then(passages => {
